...
def main() -> None:
    ...
    st.caption(
        "Synthetic local BI dashboard for campaign ROI, ROAS, CAC, attribution, funnel conversion, "
        "budget pacing, target attainment, and reporting trust."
    )
    st.caption(
        "Synthetic, privacy-safe data keeps the local BI workflow reproducible while preserving realistic "
...
def render_sidebar(data: dict[str, pd.DataFrame]) -> dict[str, object]:
    with st.sidebar:
        ...
        st.download_button(
            ...
            file_name="demo_mart_channel_performance.csv",
            mime="text/csv",
            ...
            [
                {
                    ...
                    "source": data[key].attrs.get("source_path", "missing"),
