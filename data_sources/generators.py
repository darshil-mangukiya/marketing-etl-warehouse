...
class SyntheticMarketingGenerator:
    ...
    def __init__(
        ...
        self._generated_lead_parts: list[pd.DataFrame] = []
        self._sql_closer_ids: np.ndarray | None = None
    ...
    def crm_leads(self, rows: int) -> pd.DataFrame:
        ...
        self._generated_lead_parts.append(
            frame[["lead_id", "customer_id", "created_at", "campaign_id", "qualification_stage", "cdc_operation"]].copy()
        )
        return frame

    def sales_conversions(self, rows: int) -> pd.DataFrame:
        lead_ids = np.array(
            [f"LEAD-{value:010d}" for value in self.rng.integers(1, 20000000, rows)],
            dtype=object,
        )
        customer_ids = np.array(
            [f"CUST-{value:010d}" for value in self.rng.integers(1, 12000000, rows)],
            dtype=object,
        )
        campaign_ids = self._campaign_ids(rows, missing_rate=0.09)
        dates = self._event_dates(rows).to_numpy(dtype="datetime64[ns]")

        if self._generated_lead_parts:
            leads = pd.concat(self._generated_lead_parts, ignore_index=True)
            leads = leads[leads["cdc_operation"].fillna("I").ne("D")]
            leads = leads.drop_duplicates("lead_id", keep="last").set_index("lead_id")
            sql_leads = leads[leads["qualification_stage"].eq("sales_qualified")]
            non_sql_leads = leads[leads["qualification_stage"].ne("sales_qualified")]

            if self._sql_closer_ids is None and not sql_leads.empty:
                closer_count = max(1, int(np.ceil(len(sql_leads) * 0.60)))
                self._sql_closer_ids = self.rng.choice(
                    sql_leads.index.to_numpy(dtype=object),
                    size=closer_count,
                    replace=False,
                )

            linked_sql_mask = self.rng.random(rows) < 0.80
            if self._sql_closer_ids is not None and len(self._sql_closer_ids):
                lead_ids[linked_sql_mask] = self.rng.choice(
                    self._sql_closer_ids,
                    size=int(linked_sql_mask.sum()),
                    replace=True,
                )

            linked_non_sql_mask = ~linked_sql_mask
            if not non_sql_leads.empty:
                lead_ids[linked_non_sql_mask] = self.rng.choice(
                    non_sql_leads.index.to_numpy(dtype=object),
                    size=int(linked_non_sql_mask.sum()),
                    replace=True,
                )

            linked = leads.reindex(lead_ids)
            known_lead_mask = linked.index.to_series().isin(leads.index).to_numpy()
            customer_ids[known_lead_mask] = linked.loc[known_lead_mask, "customer_id"].to_numpy(dtype=object)
            campaign_ids[known_lead_mask] = linked.loc[known_lead_mask, "campaign_id"].to_numpy(dtype=object)
            dates[known_lead_mask] = pd.to_datetime(
                linked.loc[known_lead_mask, "created_at"], errors="coerce"
            ).to_numpy(dtype="datetime64[ns]")

        unknown_mask = self.rng.random(rows) < 0.025
        lead_ids[unknown_mask] = "UNKNOWN"
        ...
        frame = pd.DataFrame(
            {
                ...
                "created_at": pd.to_datetime(dates).astype(str),
