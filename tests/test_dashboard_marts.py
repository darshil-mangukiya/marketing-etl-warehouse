...
def test_funnel_sql_to_close_uses_only_converted_sql_leads() -> None:
    leads = pd.DataFrame(
        {
            "lead_id": ["L1", "L2", "L3", "L4", "L5"],
            "created_at": ["2025-01-01"] * 5,
            "lead_source": ["Google Ads"] * 5,
            "qualification_stage": [
                "sales_qualified",
                "sales_qualified",
                "sales_qualified",
    ...
    sales = pd.DataFrame({"lead_id": ["L1", "L1", "L4", "UNKNOWN"]})

    funnel = _funnel_performance(leads, sales)

    assert funnel["sales_qualified_leads"].sum() == 3
    assert funnel["conversions"].sum() == 1
    assert funnel["sql_to_close_rate"].iloc[0] == 1 / 3
    assert funnel["sql_to_close_rate"].between(0, 1).all()
