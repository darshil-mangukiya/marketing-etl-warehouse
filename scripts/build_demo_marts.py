...
def _funnel_performance(leads: pd.DataFrame, sales: pd.DataFrame) -> pd.DataFrame:
    if leads.empty:
        return pd.DataFrame()
    frame = leads.copy()
    if "cdc_operation" in frame.columns:
        frame = frame[frame["cdc_operation"].fillna("I").ne("D")]
    frame = frame.drop_duplicates("lead_id", keep="last")
    ...
    sql_cohort = frame[frame["qualification_stage"].eq("sales_qualified")][
        ["lead_id", "reporting_month", "normalized_channel"]
    ]
    if sales.empty or sql_cohort.empty:
        grouped["conversions"] = 0
    else:
        converted_sqls = (
            sales[["lead_id"]]
            .dropna()
            .merge(sql_cohort, on="lead_id", how="inner")
            .groupby(["reporting_month", "normalized_channel"], dropna=False)
            .agg(conversions=("lead_id", "nunique"))
            .reset_index()
        )
        grouped = grouped.merge(
            converted_sqls,
            on=["reporting_month", "normalized_channel"],
            how="left",
        )
        grouped["conversions"] = grouped["conversions"].fillna(0).astype(int)
