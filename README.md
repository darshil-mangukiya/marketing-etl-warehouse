...
[![Python](https://img.shields.io/badge/Python-3.11%2B-3776AB?logo=python&logoColor=white)](https://www.python.org/) [![dbt](https://img.shields.io/badge/dbt-Analytics%20Engineering-FF694B?logo=dbt&logoColor=white)](https://www.getdbt.com/) [![Airflow](https://img.shields.io/badge/Apache%20Airflow-3.3.1-017CEE?logo=apacheairflow&logoColor=white)](https://airflow.apache.org/) [![PostgreSQL](https://img.shields.io/badge/PostgreSQL-16-4169E1?logo=postgresql&logoColor=white)](https://www.postgresql.org/) [![DuckDB](https://img.shields.io/badge/DuckDB-Local%20Warehouse-FFF000?logo=duckdb&logoColor=black)](https://duckdb.org/) [![BigQuery](https://img.shields.io/badge/BigQuery-Cloud%20Warehouse-669DF6?logo=googlebigquery&logoColor=white)](https://cloud.google.com/bigquery)<br>
[![FastAPI](https://img.shields.io/badge/FastAPI-API-009688?logo=fastapi&logoColor=white)](https://fastapi.tiangolo.com/) [![Docker](https://img.shields.io/badge/Docker-Compose-2496ED?logo=docker&logoColor=white)](https://docs.docker.com/compose/) [![Terraform](https://img.shields.io/badge/Terraform-IaC-844FBA?logo=terraform&logoColor=white)](https://developer.hashicorp.com/terraform) [![Google Cloud](https://img.shields.io/badge/Google%20Cloud-GCP-4285F4?logo=googlecloud&logoColor=white)](https://cloud.google.com/) [![Power BI](https://img.shields.io/badge/Power%20BI-Semantic%20BI-F2C811?logo=powerbi&logoColor=black)](https://learn.microsoft.com/power-bi/) [![Streamlit](https://img.shields.io/badge/Streamlit-Analytics%20App-FF4B4B?logo=streamlit&logoColor=white)](https://streamlit.io/)<br>
[![Local Quality Gate](https://img.shields.io/github/actions/workflow/status/darshil-mangukiya/marketing-etl-warehouse/local-quality-gate.yml?branch=main&label=Local%20Quality%20Gate)](https://github.com/darshil-mangukiya/marketing-etl-warehouse/actions/workflows/local-quality-gate.yml)

## At a glance

- **Six modeled ad-hoc requests** with clarification, SQL/Python, governed evidence, validation, visualization, recommendation, and stakeholder response.
- **10-slide executive PowerPoint** plus a two-minute analytics brief, both calculated from existing project outputs.
- **44 enabled local dbt models + 53 data tests**, with attribution, funnel, target, variance, quality, customer-value, and action marts.
- **28-task Airflow DAG** and governed DuckDB, PostgreSQL, and bounded BigQuery/GA4 execution paths.
- **Six attribution methodologies** with reconciliation and sensitivity analysis; no causal-lift claim.
- **Power BI + Streamlit** decision surfaces, source-controlled semantic assets, and reporting-trust controls.

## Business problem

Marketing leaders need more than a dashboard: they need an analyst who can clarify a vague request, validate the data, explain what changed, distinguish evidence from interpretation, and recommend a controlled next step. This project combines the governed marketing platform with the analyst workflow that consumes it.

## What this project demonstrates

| Role lens | Evidence |
|---|---|
| Data Analyst | Six ad-hoc investigations, variance/funnel/attribution analysis, concise response memos |
| BI / Analytics Engineering | Power BI, Streamlit, DAX, dbt marts, semantic governance, reconciliation |
| Data Engineering | Python ingestion, Airflow, DuckDB/PostgreSQL/BigQuery, testing, quality controls |

## 5-minute review

1. [Executive PowerPoint](executive_review/Marketing_Monthly_Business_Review.pptx) — management story and actions.
2. [Ad-Hoc Marketing Analysis Pack](analytics_requests/README.md) — six reproducible modeled requests.
3. [Executive Overview screenshot](evidence/screenshots/powerbi/executive_overview.png) — implemented Power BI evidence.
4. [Architecture snapshot](evidence/generated/architecture_snapshot.svg) — governed source-to-decision flow.
5. [PowerPoint validation](executive_review/validation_report.json) and [request validations](analytics_requests/manifest.yml) — provenance and release checks.

## Business questions answered

- Why is paid-social attributed ROAS below the review line, and is the gap concentrated?
- Where is the largest Lead → MQL → SQL → Close leakage?
- Why can revenue miss target when spend is near plan?
- Does the channel conclusion change across six attribution methods?
- What should be reviewed under deterministic **SIMULATED / WHAT-IF** budget cases?
- Can current recommendations be trusted, or should a **DATA QUALITY HOLD** apply?

## Key analytical capabilities

ROI / ROAS / CAC · funnel conversion · six-method attribution · target vs actual · variance drivers · customer value · data quality / source health · deterministic scenario analysis · campaign action review

## Reviewer navigation

| Reviewer wants to see | Start here |
|---|---|
| Data Analyst | [Ad-Hoc Analysis Pack](analytics_requests/README.md) |
| Executive communication | [Monthly Business Review](executive_review/README.md) / [Executive Analytics Brief](executive_review/EXECUTIVE_ANALYTICS_BRIEF.md) |
| Stakeholder workflow | [Modeled stakeholder requests](business_analysis/stakeholder_requests/README.md) |
| Recruiter | [Recruiter brief](docs/career/P2_RECRUITER_BRIEF.md) |
| Interview preparation | [Interview guide](docs/career/P2_INTERVIEW_GUIDE.md) |
| BI | [Power BI](dashboards/powerbi/README.md) / [Streamlit](bi_app/README.md) |
| Analytics Engineering | [dbt documentation](docs/dbt.md) |
| Data Engineering | [Architecture](docs/architecture.md) / Airflow DAG |
| Cloud | [Bounded GCP evidence](docs/P2_LIVE_GCP_VALIDATION.md) |
| Testing | [Evidence](evidence/README.md) / [deck validation](executive_review/validation_report.json) |

## Important boundaries

- Marketing business inputs and performance results are generated/synthetic; no real customers, ad spend, campaign performance, realized revenue impact, or realized savings are claimed.
- The bounded real portfolio-site GA4 → BigQuery path is clearly separated from generated marketing data and is not used in the executive review.
- No live Google Ads, Meta Ads, or TikTok Ads execution is performed; scenario outputs are **SIMULATED / WHAT-IF** and are not optimal or causal allocations.
- Modeled stakeholder requests are portfolio case studies, not actual meetings, adoption, approval, or UAT sign-off.
- Power BI Service deployment, production operation, enterprise SLA ownership, and production-scale 38M-row processing are not claimed.

Detailed technical documentation continues below.

| Engineering check | Verified result |
|---|---:|
| Python quality | **112 tests passing · Ruff 0 findings** |
| Local dbt build | **97/97 operations** |
...
| Quality and security gates | **26/26 checks · 0 known dependency vulnerabilities · CI passing** |
...
- **The platform has real runtime depth.** A 28-task Airflow DAG, a nine-service Docker Compose topology, PostgreSQL, DuckDB, and cost-controlled BigQuery paths exercise more than static SQL files.
...
The DuckDB build executes **44 enabled models + 53 data tests = 97/97 successful operations**. Four live-GA4 models are BigQuery-specific—nested event extraction, repeated ecommerce items, privacy-conscious sessionization, and the live funnel—and are intentionally excluded from the local DuckDB graph.
...
Docker Compose defines **nine services** spanning PostgreSQL, Airflow initialization/API server/scheduler/DAG processor, ingestion, the FastAPI simulator, Streamlit, and dbt.
...
| Local validation | 112 pytest tests, 97/97 dbt operations, 26/26 quality-gate checks, and 0 known dependency vulnerabilities |
| Repository | Source-controlled implementation, validation evidence, and reproducible local workflows |
