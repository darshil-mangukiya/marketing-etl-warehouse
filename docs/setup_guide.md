...
Airflow is intentionally pinned to the Docker runtime (`apache/airflow:2.9.3-python3.11`). The default `requirements.txt` supports local tests, release-site generation, dbt parsing, and the API simulator on modern local Python versions. Install `requirements-airflow.txt` only in a Python 3.11 environment when you need to run Airflow outside Docker.
